...
require_once './src/models/Barco.php';
...
function obtenerInventos(): array{
	...
	$numeroMinimo = $_COOKIE['minRegistros'] ?? 0 ;
	
	foreach ($inventos as $invento => $_) {
        if (isset($_SESSION[$invento])) {
            $inventoSer = $_SESSION[$invento];
            $inventoDes = [];
            foreach ($inventoSer as $_ => $inventoSerializado) {
                $inventoDes[] = unserialize($inventoSerializado);
            }
			if(count($inventoDes) < $numeroMinimo){
				$diferencia = $numeroMinimo - count($inventoDes);
				$materiales = obtenerMateriales();
				$zonas = obtenerZonas();
				$tecnicas = obtenerTecnicas();
				$inventoNuevo = generarInvento($invento, $diferencia , $inventos, $materiales, $zonas, $tecnicas);
				$inventoDes = array_merge($inventoDes, $inventoNuevo);
			}
...
function cargarVistaTipoInventos(): void{
	$estadoBorrarInventos = isset($_GET['borrar']);
	$minInventos = isset($_COOKIE['minInventos']) ? (int)$_COOKIE['minInventos'] : 0;
	
	if($estadoBorrarInventos != null && $estadoBorrarInventos === true){
		limpiarSesionInventos();
	}

	$html = '<h1>Tipos de Inventos</h1>';
	$html .= '<table class="invento-table">';
    $html .= '<tr><th>Nombre del Invento</th><th>Acciones';
	$html .= '<a href="./index.php?borrar=true" name="borrarCookie">Borrar Inventos</a>';
	$html .= '</th></tr>';
	//punto 2
	$html .= '<tr>';
    $html .= '<td>Número mínimo de registros:</td>';
    $html .= '<td>';
    $html .= '<form action="cookieController.php" method="POST">';
    $html .= '<input type="number" name="minRegistrosInput" value="' . htmlspecialchars($minInventos) . '" min="0" required>';
    $html .= '<button type="submit">Guardar</button></form>';
	$html .= '<form action="cookieController.php" method="GET">';
    $html .= '<a href="./cookieController.php?borrar=true" name="borrar"> Borrar</a>';
    $html .= '</form>';
    $html .= '</td>';
    $html .= '</tr>';
	...
	if (isset($nombreInvento) && array_key_exists($nombreInvento, obtenerInventos())) {
		switch ($nombreInvento) {
			...
			case 'Barco':
				...
				for ($i=0; $i < 3; $i++) { 
					$telas[] = $inventos['Tela'][$datosEntrada['tela'][$i]];
				}	
				for ($i=0; $i < 10; $i++) { 
					$cuerdas[] = $inventos['Cuerda'][$datosEntrada['cuerda'][$i]];
				}	
				$hacha = $inventos['Hacha'][$datosEntrada['hacha'][0]];
				$carro = $inventos['Carro'][$datosEntrada['carro'][0]];
				$nombre = $datosEntrada['nombre'];
				$tamanyo = [
					'longitud' => $datosEntrada['longitud'], 
					'ancho' => $datosEntrada['ancho'], 
				...
				$zona = $datosEntrada['zona'];
				$tecnica = $datosEntrada['tecnica'];
				$numeroElemento = $datosEntrada['numeroElementos'];
				...
				for($i = 2; $i >= 0; $i--){
					eliminarInvento('Tela', $datosEntrada['tela'][$i]);
				}
				for ($i=9; $i >= 0; $i--) {
					eliminarInvento('Cuerda', $datosEntrada['cuerda'][$i]);
				}
				...
				eliminarInvento('Hacha', $datosEntrada['hacha'][0]);
				eliminarInvento('Carro', $datosEntrada['carro'][0]);
...
function generarInvento(string $nombre, int $numero, array $inventos, array $materiales, array $zonas, array $tecnicas)
{
	...
	for ($i = 0; $i < $numero; $i++) {
		$inventosGenerados[] = match ($nombre) {
			'PiedraAfilada' => new PiedraAfilada(obtenerObjetoAleatorio($materiales), 'PiedraAfilada' . $i, ['base' => mt_rand(1000, 10000) / 100, 'altura' => mt_rand(1000, 10000) / 100, 'longitud' => mt_rand(1000, 10000) / 100], obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			'Cuerda'        => new Cuerda(obtenerObjetoAleatorio($materiales), 'Cuerda' . $i, ['radio' => mt_rand(100, 1000) / 100, 'altura' => mt_rand(1000, 10000) / 100], obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			'Fuego'         => new Fuego(obtenerObjetoAleatorio($materiales), 'Fuego' . $i, obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			'Lanza'         => new Lanza(obtenerObjetoAleatorio($materiales), obtenerObjetoAleatorio($inventos['PiedraAfilada']), obtenerObjetoAleatorio($inventos['Cuerda']), 'Lanza' . $i, ['radio' => mt_rand(100, 1000) / 100, 'altura' => mt_rand(1000, 10000) / 100], obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			'ArcoFlecha'    => new ArcoFlecha(obtenerObjetoAleatorio($materiales), obtenerObjetoAleatorio($inventos['Lanza']), obtenerObjetoAleatorio($inventos['Cuerda']), 'ArcoFlecha' . $i, 'tallado', ['radio' => mt_rand(100, 1000) / 100], obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			'Cesta'         => new Cesta(obtenerObjetoAleatorio($materiales), 'Cesta' . $i, ['radio' => mt_rand(1000, 10000) / 100, 'altura' => mt_rand(1000, 10000) / 100, 'grosor' => mt_rand(100, 1000) / 100], obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			'Refugio'       => new Refugio(obtenerObjetoAleatorio($materiales), obtenerObjetoAleatorio($materiales), obtenerObjetoAleatorio($materiales), 'Refugio' . $i, ['ancho' => mt_rand(1000, 10000) / 100, 'altura' => mt_rand(1000, 10000) / 100, 'longitud' => mt_rand(1000, 10000) / 100, 'grosor' => mt_rand(100, 1000) / 100], obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			'Rueda'         => new Rueda(obtenerObjetoAleatorio($materiales), 'Rueda' . $i, ['radio' => mt_rand(1000, 10000) / 100, 'altura' => mt_rand(1000, 10000) / 100], obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			'Trampa'        => new Trampa(obtenerObjetoAleatorio($inventos['Cuerda']), obtenerObjetoAleatorio($inventos['Cesta']), obtenerObjetoAleatorio($inventos['ArcoFlecha']), 'Trampa' . $i, mt_rand(100, 1000) / 100, obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			'Hacha'         => new Hacha(obtenerObjetoAleatorio($materiales), obtenerObjetoAleatorio($inventos['PiedraAfilada']), 'Hacha' . $i, ['radio' => mt_rand(1000, 10000) / 100, 'altura' => mt_rand(1000, 10000) / 100], obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			...
			'Torno'         => new Torno(obtenerObjetoAleatorio($inventos['Alfareria']), obtenerObjetoAleatorio($inventos['Ceramica']), obtenerObjetoAleatorio($inventos['Rueda']), 'Torno' . $i, mt_rand(100, 1000) / 100, mt_rand(100, 1000) / 100, obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			'Agricultura'   => new Agricultura(obtenerObjetoAleatorio($materiales), array_fill(0, 10, obtenerObjetoAleatorio($inventos['Cesta'])), array_fill(0, 20, obtenerObjetoAleatorio($inventos['Lanza'])), obtenerObjetoAleatorio($inventos['Ganaderia']), 'Agricultura' . $i, ['longitud' => mt_rand(1000, 10000) / 100, 'ancho' => mt_rand(1000, 10000) / 100, 'altura' => mt_rand(1000, 10000) / 100, 'grosor' => mt_rand(100, 1000) / 100], obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			...
			'Fermentacion'  => new Fermentacion(obtenerObjetoAleatorio($materiales), [obtenerObjetoAleatorio($inventos['Alfareria']), obtenerObjetoAleatorio($inventos['Alfareria'])], obtenerObjetoAleatorio($inventos['Torno']), [obtenerObjetoAleatorio($inventos['Agricultura']), obtenerObjetoAleatorio($inventos['Agricultura'])], 'Fermentacion' . $i, mt_rand(3600, 86400), mt_rand(86401, 172800), obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
			...
			'Barco'         => new Barco(obtenerObjetoAleatorio($materiales), array_fill(0, 3, obtenerObjetoAleatorio($inventos['Tela'])), array_fill(0, 10, obtenerObjetoAleatorio($inventos['Cuerda'])), obtenerObjetoAleatorio($inventos['Hacha']), obtenerObjetoAleatorio($inventos['Carro']), 'Barco' . $i, ['longitud' => mt_rand(20000, 100000) / 100, 'ancho' => mt_rand(20000, 50000) / 100, 'altura' => mt_rand(5000, 20000) / 100, 'grosor' => mt_rand(300, 1000) / 100], obtenerObjetoAleatorio($zonas), obtenerObjetoAleatorio($tecnicas)),
		};
	}
...
function procesarCookie(){
	$borradoCookie = $_GET['borrar'];
	$numeroMinimo = $_POST['minRegistrosInput'];
	
	if($borradoCookie != null && $borradoCookie == true){
		
		limpiarCookie();
	}
	if(is_numeric($numeroMinimo) && $numeroMinimo > 0){
		setcookie('minRegistros', (int)$numeroMinimo, time()+86400);
	}
	header('Location: ./index.php');
	exit();
}
function limpiarCookie(){
	setcookie('minRegistros', "", time()-1);
	unset($_COOKIE['minRegistros']);
